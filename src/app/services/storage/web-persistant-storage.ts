...
export class WebPersistantStorage extends PersistantStorage {
    ...
    get<T>(key: string): Promise<T | undefined> {
        ...
        return Promise.resolve(value ? (JSON.parse(value) as { value: T }).value : undefined);
    }
    ...
    entries<T>(): Promise<[key: string, value: T][]> {
        ...
        return Promise.resolve(entries.map(([key, value]) => [key, (JSON.parse(value) as { value: T }).value]));
    }
